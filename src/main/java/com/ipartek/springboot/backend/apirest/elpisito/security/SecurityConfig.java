...
public class SecurityConfig {
	...
	SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		...
		/*
		 * CORS (Cross-Origin Resource Sharing) es un mecanismo de seguridad de los
		 * navegadores web que utiliza cabeceras HTTP para permitir o restringir que un
		 * sitio web (dominio A) acceda a recursos de otro dominio diferente (dominio
		 * B). Evita que scripts maliciosos carguen datos sensibles de otros sitios sin
		 * autorización, siendo esencial en el desarrollo web moderno para peticiones
		 * AJAX/Fetch entre API distintas.
		 */
 ...
 * CorsConfiguration config = new CorsConfiguration();
 * 
 * config.setAllowedOrigins(List.of("http://localhost:4200"));//Dominio desde
 * donde se sirve la Web y otros dominios que nos interesen
 * config.setAllowedMethods(List.of("GET","POST","PUT","PATCH","DELETE"));
 * //Solo podemos utilizar en las peticiones los "verbos" (métodos)
 * especificados
 * config.setAllowedHeaders(List.of("Authorization","Cache-Control",
 * "Content-Type")); config.setAllowCredentials(true);
 * config.setExposedHeader(List.of("Authorization")); //De esta manera podemos
 * acceder a la información de "Authorization" en un interceptor en Angular
 * 
